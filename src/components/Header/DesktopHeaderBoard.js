  ...
  return (
    <Box sx={{minHeight: '400px', padding: 2, borderTop: '1px solid grey'}} >
        <Grid container spacing={1}>
            ...
            <Grid item xs= {6}>
                <Box>
                    ...
                    list.map(item => {
                        ...
                        return(
                            <Grid item xs= {isLong? 6 : 12} key={item.nameUZB + item.nameRUS}>
                                <Box  marginTop={1} sx={{cursor: 'pointer'}} onClick = {() => navigateUserToPage(item.id)}>
            ...
            <Grid item xs= {4} sx={{display: 'flex', alignItems: 'center', height: '100%'}}>
